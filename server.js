    ...
    ws.on('message', (raw) => {
        ...
        switch (msg.type) {
            ...
            case 'create_room': {
                ...
                const room = {
                    id: roomId, name: msg.roomName || 'Room',
                    hostPlayerId: playerId, password: msg.password || '',
                    isPrivate: !!msg.isPrivate, players: [],
                    map: msg.map || 'Hospital', teamSize: msg.teamSize || 2,
                    inGame: false, createdAt: Date.now()
                };
                player.roomId = roomId;
                player.ready = false;
                room.players.push(player);
                ...
                sendTo(ws, {
                    ...
                    roomName: room.name, teamSize: room.teamSize,
                    map: room.map, hasPassword: room.password !== ''
                });
                sendTo(ws, {
                    type: 'room_joined', roomId: roomId,
                    roomName: room.name, teamSize: room.teamSize,
                    map: room.map,
                    players: room.players.map(p => ({
                        id: p.id, name: p.name, ready: p.ready || false
                    })),
                    hostId: room.hostPlayerId
                });
                break;
            ...
            case 'quick_match': {
                ...
                if (!found) {
                    ...
                    const room = {
                        ...
                        map: 'Hospital', teamSize: 2, inGame: false, createdAt: Date.now()
                    };
                    player.roomId = roomId;
                    player.ready = false;
                    room.players.push(player);
            ...
            case 'player_damage': {
                if (!player.roomId) break;
                broadcastToRoom(player.roomId, {
                    type: 'player_damage', playerId: msg.targetId,
                    damage: msg.damage, sourceId: playerId
                });
                break;
            }

            case 'toggle_ready': {
                if (!player.roomId) { sendError(ws, 'Not in room'); break; }
                const rdyRoom = rooms.get(player.roomId);
                if (!rdyRoom || rdyRoom.inGame) { sendError(ws, 'Cannot ready'); break; }
                player.ready = !player.ready;
                broadcastToRoom(player.roomId, {
                    type: 'player_ready_changed', playerId: playerId, ready: player.ready
                });
                break;
            }
            ...
            case 'start_match': {
                if (!player.roomId) { sendError(ws, 'Not in room'); break; }
                const stRoom = rooms.get(player.roomId);
                if (!stRoom) break;
                if (stRoom.hostPlayerId !== playerId) { sendError(ws, 'Only host'); break; }
                if (stRoom.inGame) { sendError(ws, 'Already started'); break; }
                if (stRoom.players.length < 2) { sendError(ws, 'Need 2+ players'); break; }
                if (!checkAllReady(stRoom)) { sendError(ws, 'Not ready'); break; }
                stRoom.inGame = true;
                broadcastToRoom(player.roomId, {
                    type: 'game_start',
                    players: stRoom.players.map(p => ({ id: p.id, name: p.name })),
                    map: stRoom.map, teamSize: stRoom.teamSize
                });
                break;
            }
            ...
            case 'kick_player': {
                if (!player.roomId) break;
                const kRoom = rooms.get(player.roomId);
                if (!kRoom || kRoom.hostPlayerId !== playerId) break;
            ...
            case 'transfer_host': {
                if (!player.roomId) break;
                const tRoom = rooms.get(player.roomId);
                if (!tRoom || tRoom.hostPlayerId !== playerId) break;
                const newHost = players.get(msg.targetId);
                if (newHost && newHost.roomId === player.roomId) {
                    tRoom.hostPlayerId = msg.targetId;
                    broadcastToRoom(player.roomId, {
                        type: 'host_changed', newHostId: msg.targetId, newHostName: newHost.name
                    });
                }
                break;
            }
